 ...
 * Throws a descriptive error if the variable is missing, so tests fail
 * immediately with a clear message rather than silently using `undefined`.
 */
function getEnv(key: string): string {
  const value = process.env[key];
  if (!value) {
    const msg = `[auth.data] Missing required environment variable: "${key}". ` +
      'Copy frontend/.env.example → frontend/.env and provide all values.';
    Logger.error(msg);
    throw new Error(msg);
  }
  return value;
